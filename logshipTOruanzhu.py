...
@st.cache_data
def load_data():
    url = "https://github.com/Jane-zzz-123/TOruanzhu/raw/main/LogTOruanzhu.xlsx"
    try:
...
region_col = "区域"  # 区域列名
ship_method_col = "物流方式"  # 物流方式列名
forwarder_stage_cols = [
    "开船-到港","到港-提柜","提柜-签收"
]
warehouse_stage_col = "签收-完成上架"  # 仓库负责的环节
...
if total_delay == 0:
    ...
    st.markdown(f"""
        ...
        #### 📌 提柜-签收环节物流标准
        - 常规物流：区域A≤6天 | 区域B≤10天 | 区域C≤11天
        - 渠道C（专属）：区域C≤4天
        #### 📌 仓库环节物流标准
        - 签收-完成上架：≤3天
        """)
    # 6. 各环节耗时均值对比（正常 vs 延期，按物流方式+区域）
...
if len(df_warehouse_valid) == 0:
    ...
    df_warehouse_valid[WAREHOUSE_COLUMN_MAPPING["区域列名"]] = (
        ...
        .astype(str)
        .str.strip()
        .replace({"East": "区域C", "Mid": "区域B", "West": "区域A"})
    )
    # ===== 1. 核心计算 =====
    ...
    with col_east:
        st.markdown("#### 区域C区域")
        if summary_by_region["区域C"]:
            st.markdown("\n".join(summary_by_region["区域C"]), unsafe_allow_html=True)
        else:
            st.markdown("暂无区域C区域仓库数据")
    with col_mid:
        st.markdown("#### 区域B区域")
        if summary_by_region["区域B"]:
            st.markdown("\n".join(summary_by_region["区域B"]), unsafe_allow_html=True)
        else:
            st.markdown("暂无区域B区域仓库数据")
    with col_west:
        st.markdown("#### 区域A区域")
        if summary_by_region["区域A"]:
            st.markdown("\n".join(summary_by_region["区域A"]), unsafe_allow_html=True)
        else:
            st.markdown("暂无区域A区域仓库数据")
    # 下载
